...
public class DeviceSensor implements SensorEventListener {
    ...
    public void enableIntervalSendMessage(){
        Log.i(TAG, ".enableIntervalSendMessage() entered");
        if(!isEnabled){
            //sensorManager.registerListener(this, accelerometer, SensorManager.SENSOR_DELAY_NORMAL);
            //sensorManager.registerListener(this, magnetometer, SensorManager.SENSOR_DELAY_NORMAL);
            tripId = System.currentTimeMillis()/1000;
            timer = new Timer();
            timer.scheduleAtFixedRate(new SendTimerMessageTask(), 1000, 1000);
            isEnabled = true;
        }
    }
    /**
    ...
    /**
     * Timer task for sending string message data on 1000ms intervals
     */
    private class SendTimerMessageTask extends  TimerTask{
        @Override
        public void run(){
